...
class Event_content_model extends CI_Model
{
	...
	public function get_Totalrequest() {
        ...
        $sql	= "	SELECT m.* FROM M_grand_prize_event m $where_sql Order By m.id asc ";
   	...
   	public function get_category() {
        ...
        $sql	= "	SELECT m.* FROM M_grand_prize_event m $where_sql Order By m.id desc LIMIT $offset,$rows";
        $query		= $this->db->query($sql);
        $data		= $query->result_array();
        return $data;
    }
	...
	public function select_category($id='', $name='') {
        ...
        $sql	= "	SELECT * FROM M_grand_prize_event".$where_sql;
        $query		= $this->db->query($sql);
        $data		= $query->result_array();
        return $data;
	}
