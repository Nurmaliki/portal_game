    ...
    public function report_member() {
		...
		public function akt_login_user( $from_date='', $to_date='') {
					$where_sql	= '';
							...
							if($from_date != '' and $to_date != ''){
							$where_sql	.= ' and (date >= "'.$from_date.'" and date <= "'.$to_date.'")';
							}
					$sql	= "SELECT * FROM `g_aktifitas_userapps` JOIN g_appusers ON g_appusers.id =g_aktifitas_userapps.id_appusers WHERE type LIKE 'login'".$where_sql;
					$query		= $this->db->query($sql);
					$data		= $query->result_array();
					return $data;
			}
