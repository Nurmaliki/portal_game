...
class News_category extends MY_Controller {
	...
	public function datatables()
	{
		...
		$get_Totalrequest		= $this->news_model->get_Totalrequest();
		$data 		= array();
		if(count($category) > 0){
			...
			$nestedData[] 	= $category[$i]["name"];
				if($this->session->userdata['user_data_web']['role'] == 1 or $this->session->userdata['user_data_web']['id'] == 2){
					$nestedData[] 	= '<a href="'.$this->config->item('base_url').'news_category/update/'.$category[$i]["id"].'" class="fa fa-fw fa-pencil">&nbsp;</a>&nbsp;<a href="'.$this->config->item('base_url').'news_category/delete/'.$category[$i]["id"].'" class="fa fa-fw fa-trash" data-confirm="Are you sure you want to Delete this data?">&nbsp;</a>';
					$data[] = $nestedData;
				}else if($this->session->userdata['user_data_web']['role'] == 3){
					$nestedData[] 	= '<a href="'.$this->config->item('base_url').'news_category/update/'.$category[$i]["id"].'" class="fa fa-fw fa-pencil">&nbsp;</a>';
                    $data[] = $nestedData;
    ...
    public function create()
	{
		$parseData ['header']			= $this->load->view ( 'header', '', true);
		$parseData ['left_coloumn']		= $this->load->view ( 'left_coloumn', '', true);
		$parseData ['content']			= $this->load->view ( 'content/news_category_create', '', true);
		$parseData ['footer']			= $this->load->view ( 'footer', '', true);
		$parseData ['control_sidebar']	= $this->load->view ( 'control_sidebar', '', true);
		$this->load->view ( 'vside', $parseData);
	}
    ...
    public function update($id)
	{
		...
		$data['category']				= $this->news_model->select_category($data['id'], '');
		$parseData ['header']			= $this->load->view ( 'header', '', true);
		$parseData ['left_coloumn']		= $this->load->view ( 'left_coloumn', '', true);
		$parseData ['content']			= $this->load->view ( 'content/news_category_update', $data, true);
		$parseData ['footer']			= $this->load->view ( 'footer', '', true);
		$parseData ['control_sidebar']	= $this->load->view ( 'control_sidebar', '', true);
		$this->load->view ( 'vside', $parseData);
    }
	...
	public function action_update() 
	{
		...
		if ($this->form_validation->run() == TRUE){
			$category	= $this->news_model->select_category('', $data['name']);
			if(count($category) > 0){
			$this->session->set_flashdata('msgalert', 'Update data Failed, Category already exists');
			header("location: ".$this->config->item('base_url')."news_category/update/".$data['id']);
			die;
			}else{
				$field 	= array("name" => $data['name']);
				$update_province	= $this->crud_model->update("M_news_cat", $field, $data['id']);
				$this->session->set_flashdata('msgalert', 'Update success');
				header("location: ".$this->config->item('base_url')."news_category");
				die;
			}
		}else{
	...
	public function action_create()
	{
		...
		if ($this->form_validation->run() == TRUE){
			...
			if(count($category) > 0){
                ...
                header("location: ".$this->config->item('base_url')."news_category/create");
                die;
			}else{
				$field 		= array("name" => $data['name']);
				$create_admin	= $this->crud_model->create("M_news_cat", $field);
				if($create_admin){
	...
	{
		$data['id']	= ($id != '' && is_numeric($id))? $id:0;
		$delete_admin	= $this->crud_model->delete("M_news_cat", array("id" => $id));
		if($delete_admin){
