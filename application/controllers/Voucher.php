...
class Voucher extends MY_Controller {
    ...
    public function datatables()
    {
            ...
            if(count($member) > 0){
                ...
                // $nestedData[] 	= $member[$i]["point"];
                    ...
                    if($this->session->userdata['user_data_web']['role'] == 1 ){
                    ...
                    }else if($this->session->userdata['user_data_web']['id'] == 2){
                    ...
                    }else if($this->session->userdata['user_data_web']['role'] == 4 ){
	...
	public function create()
	{
        $this->form_validation->set_rules('name', 'Name', 'required',
                array('required' => 'Anda belum mengisi nama.')
        );
        $this->form_validation->set_rules('description', 'Description', 'required',
        array('required' => 'Anda belum mengisi deskripsi.')
        );
        if (empty($_FILES['userfile']['name']))
        {
            $this->form_validation->set_rules('userfile', 'Userfile', 'required',
            array('required' => 'Anda belum mengisi foto.')
            );
        }
        $this->form_validation->set_rules('category', 'Category', 'required',
        array('required' => 'Anda belum mengisi  category.')
        );
        // $this->form_validation->set_rules('value', 'Value', 'required',
        // array('required' => 'Anda belum mengisi  value.')
        // );
        $this->form_validation->set_rules('price', 'Price', 'required',
        array('required' => 'Anda belum mengisi price.')
        );
        $this->form_validation->set_rules('points', 'Points', 'required',
        array('required' => 'Anda belum mengisi points.')
        );
        // $this->form_validation->set_rules('tc', 'TC', 'required',
        // array('required' => 'Anda belum mengisi tc.')
        // );
        ...
        {
            $saveVoucherCode = false;
            $saveDataMerchant = false;
            $newid = hexdec(uniqid());
            ...
            if ($this->upload->do_upload('userfile')) {
                $successUpload = $this->upload->data("file_name");
            } else {
                $json_data = array(
                    "status"     => true,   // for every request/draw by clientside , they send a number as a parameter, when they recieve a response/data they first check the draw number, so we are sending same number in draw.
            ...
            $arrParams2['id'] = $id;
            $arrParams2['dmo_id'] = $create_id;
            $arrParams2['name'] = $this->input->post()['name'];
            $arrParams2['points'] = str_replace(',','', str_replace('.','', $this->input->post()['points']));
            $arrParams2['category'] = $this->input->post()['category'];
            $arrParams2['price'] = str_replace(',','',  str_replace(".","", $this->input->post()['price']));
            $arrParams2['description'] = $this->input->post()['description'];
            $arrParams2['tc'] = $this->input->post()['tc'];
            $arrParams2['type'] = 2;
            $arrParams2['value'] = str_replace(',','', str_replace(".","", $this->input->post()['value']));
            $arrParams2['country'] = 'ID';
            $arrParams2['img'] = "uploads/" . $this->upload->data("file_name");
            unset($arrParams['voucher_code']);
            $saveDataMerchant = $this->crud_model->create("M_merchant_btn", $arrParams2);
            if($saveDataMerchant) {
                $json_data = array(
                    "status"     => true,   // for every request/draw by clientside , they send a number as a parameter, when they recieve a response/data they first check the draw number, so we are sending same number in draw.
                ...
                $json_data = array(
                    "status"            => false,   // for every request/draw by clientside , they send a number as a parameter, when they recieve a response/data they first check the draw number, so we are sending same number in draw.
                    "message"    => 'Data merchant gagal tersimpan'
                    );
                echo json_encode($json_data);
            }
        }
    ...
    public function action_update_point()
	{
       ...
       // $addExpire = date('Y-m-d H:i:s',$addExpire);
        ...
        $creator_name=$this->session->userdata['user_data_web']['name'];
        $creator_id=$this->session->userdata['user_data_web']['id'];
...
// query update poin nov 8
    ...
    public function checkCSV()
    {
        ...
        $data =  $this->csvreader->get_header_csv($_FILES['csvfile']['tmp_name']);
				print_r($file_ext);
				print_r($data);
				die();
    ...
    public function get_delimiter_csv()
    {
        $file_ext = pathinfo($_FILES["csvfile"]["name"], PATHINFO_EXTENSION);
        $data =  $this->csvreader->get_delimiter_csv($_FILES['csvfile']['tmp_name']);
        $json_data = array(
            "status"     => true,
            "message"    => 'Data file',
            "ext" => $file_ext,
    ...
    public function input()
	{
        ...
        $invalidMerchant = array();
        $runInput = false;
        if($arrParams['voucher_code'] !== ''){
            if(date("Y-m-d") < $arrParams['expired_date'] && $arrParams['expired_date'] !== '' && $this->merchantgiift_model->check_duplicate($arrParams['merchant_id'], $arrParams['voucher_code'])){
                $dataInsertToDatabase = array(
                    'voucher_code' => $this->encription_model->encrypt($arrParams['voucher_code']),
                    'merchant_id' =>  $arrParams['merchant_id'],
                    'md5_signature' => md5($arrParams['voucher_code']),
                    'status' => 1,
                    'created_at' => date('Y-m-d'),
                    'expired_date' => $arrParams['expired_date'],
                );
                $runInput = $this->crud_model->create("M_btn_voucher_code", $dataInsertToDatabase);
            }
            if($runInput) {
                $json_data = array(
                    "status"     => true,
                ...
                $json_data = array(
                    "status"     => true,
                    "message"    => 'Data gagal disimpan, data duplikasi atau expired_date tidak valid'
                    );
                    echo json_encode($json_data);
            }
        } else {

            $dataFromCSV = $this->csvreader->parse_csv($_FILES['csvfile']['tmp_name']);
						// print_r($dataFromCSV);
						// die();

            if( count($dataFromCSV) > 0 ){
                foreach ($dataFromCSV as $value){
                    // $merchantid = $arrParams['merchant_id'] ? $arrParams['merchant_id'] : $this->merchantgiift_model->get_merchant_btn_by_id($value['merchant'])[0]['dmo_id'];
                    if($this->merchantgiift_model->check_duplicate($arrParams['merchant_id'], $value['voucher_code']) && $value['expired_date'] !== '' && date("Y-m-d") < $value['expired_date'] && $value['voucher_code'] !== '' && count($this->merchantgiift_model->get_merchant_btn_by_id($arrParams['merchant_id'])) > 0){
                        $dataInsertToDatabase = array();
                        $dataInsertToDatabase = array(
                            'voucher_code' => $this->encription_model->encrypt($value['voucher_code']),
                            'merchant_id' =>  $arrParams['merchant_id'],
                            'md5_signature' => md5($value['voucher_code']),
                            'status' => 1,
                            'created_at' => date('Y-m-d'),
                            'expired_date' => $arrParams['expired_date'] ? $arrParams['expired_date'] : $value['expired_date'],
                        );
                        $runInput = $this->crud_model->create("M_btn_voucher_code", $dataInsertToDatabase);
                        if($runInput) {
                            $countInput++;
                        ...
                        array_push($invalidMerchant, $this->merchantgiift_model->get_merchant_btn_by_id($arrParams['merchant_id'])[0]['name']);
                    }
                }
            }else{
							$json_data = array(
									"status"     => true,   // for every request/draw by clientside , they send a number as a parameter, when they recieve a response/data they first check the draw number, so we are sending same number in draw.
									"message"    => 'Data gagal disimpan. Data Kosong.'
									);
						}
                if(count($invalidMerchant) > 0 ){
                    $dataNotValid = implode(", ",$invalidMerchant);
                    if(count($dataFromCSV) == count($invalidMerchant))
                    {
                        $json_data = array(
                            "status"     => true,   // for every request/draw by clientside , they send a number as a parameter, when they recieve a response/data they first check the draw number, so we are sending same number in draw.
                            "message"    => 'Data gagal disimpan. Data duplikasi atau expired_date tidak valid.'
                        ...
                        $json_data = array(
                            "status"     => true,   // for every request/draw by clientside , they send a number as a parameter, when they recieve a response/data they first check the draw number, so we are sending same number in draw.
                            "message"    => 'Merchant ' . $dataNotValid . ' gagal disimpan. Data duplikasi atau expired_date tidak valid.'
                        );
                    }
