...
class Hadiah extends MY_Controller
{
	...
	public function __construct()
	{
		...
		// echo $this->session->userdata['user_data_web']['token'];
		// echo "<br>";
		// echo $user_token[0]['token'];
		// echo ($user_token[0]['token'] != $this->session->userdata['user_data_web']['token']) ? "tidak sama" : "sama";
		if ($this->session->userdata['user_data_web']['token'] != $user_token[0]['token']) {

			echo "tidak sama gess";
	...
	public function index()
	{
		...
		if (!isset($_GET['access']) || !$enc->stringEncryption("decrypt", $_GET['access'])) {
			...
			$parseData['content']			= $this->load->view('new_game/content/hadiah', $data, true);
	...
	public function penukaran_hadiah($id)
	{
		...
		if (!isset($_GET['access']) || !$enc->stringEncryption("decrypt", $_GET['access'])) {
			...
			$parseData['content']			= $this->load->view('new_game/content/hadiah_detail', $data, true);
			$this->load->view('new_game/vside', $parseData);
		}
	}
	...
	public function penukaran_hadiah_act_api()
	{
		...
		if ($data) {
			if ($data['jumlah'] < 0) {
				...
				$url = $this->config->item('url_api_notif') . "?k=jetset2022&m=" . $_POST["hp"] . "&p=QUOTA";
				...
				if ($data['poin'] > $data_users[0]["poin"]) {
					...
					$url = $this->config->item('url_api_notif') . "?k=jetset2022&m=" . $_POST["hp"] . "&p=INSUF";
					curl_setopt($ch, CURLOPT_URL, $url);
					curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
					$feridisini = curl_exec($ch);
					$output = json_decode($feridisini, true);
					...
					if ($jumlah_katalog_kurang) {
						...
						$url = $this->config->item('url_api_notif') . "?k=jetset2022&m=" . $_POST["hp"] . "&p=" . $data['prize_code'] . "";
						curl_setopt($ch, CURLOPT_URL, $url);
						curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
						$feridisini = curl_exec($ch);
						$output = json_decode($feridisini, true);
						...
						$sukses = array(
							'status' => true,
							'message' => 'Selamat, Poin Anda telah di tukar dengan ' . $data['name'],
						);
		...
		if (!isset($_GET['access']) || !isset($enc->stringEncryption("decrypt", $_GET['access'])->id)) {
			...
			if ($data) {
				if ($data['jumlah'] <= 0) {
					...
					$url = $this->config->item('url_api_notif') . "?k=jetset2022&m=" . $this->session->userdata['user_data_web']['username'] . "&p=QUOTA";
					curl_setopt($ch, CURLOPT_URL, $url);
					curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
					$feridisini = curl_exec($ch);
					$output = json_decode($feridisini, true);
					...
					if ($data['poin'] > $this->session->userdata['user_data_web']['poin']) {
						...
						$url = $this->config->item('url_api_notif') . "?k=jetset2022&m=" . $this->session->userdata['user_data_web']['username'] . "&p=INSUF";
						curl_setopt($ch, CURLOPT_URL, $url);
						curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
						$feridisini = curl_exec($ch);
						$output = json_decode($feridisini, true);
						...
						if ($jumlah_katalog_kurang) {
							...
							$url = $this->config->item('url_api_notif') . "?k=jetset2022&m=" . $this->session->userdata['user_data_web']['username'] . "&p=" . $data['prize_code'] . "";
							curl_setopt($ch, CURLOPT_URL, $url);
							curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
							$feridisini = curl_exec($ch);
							$output = json_decode($feridisini, true);
	...
	public function hadiah_success()
	{
		...
		if (!isset($_GET['access']) || !isset($enc->stringEncryption("decrypt", $_GET['access'])->id)) {
			...
			$parseData['content']			= $this->load->view('new_game/content/hadiah_success', '', true);
