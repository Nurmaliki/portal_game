...
class Category_model extends CI_Model
{
	...
	$sql	= "	SELECT m.* FROM M_category m $where_sql Order By m.id asc ";
	...
	$sql	= "	SELECT m.* FROM M_category m $where_sql Order By m.id desc LIMIT $offset,$rows";
	$query		= $this->db->query($sql);
	$data		= $query->result_array();
	return $data;
	}
	...
	$sql	= "	SELECT * FROM M_category".$where_sql;
	$query		= $this->db->query($sql);
	$data		= $query->result_array();
	return $data;
	}
